...
public interface RideRepository extends JpaRepository<Ride, Long> {
    ...
    @Query(value = "SELECT ride FROM Ride ride WHERE ride.driver.email=:driverEmail AND ride.actualStart BETWEEN :startOfTheDay AND :endOfTheDay")
    List<Ride> findRideByDriverEmailAndDateRange(String driverEmail, LocalDateTime startOfTheDay, LocalDateTime endOfTheDay);

    @Query(value = "SELECT ride FROM Ride ride WHERE ride.actualStart BETWEEN :startOfTheDay AND :endOfTheDay")
    List<Ride> findRideByDateRange(LocalDateTime startOfTheDay, LocalDateTime endOfTheDay);

    @Query(value = "SELECT ride FROM Ride ride JOIN ride.customers c WHERE c.email=:customerEmail AND ride.actualStart BETWEEN :startOfTheDay AND :endOfTheDay")
